...
filtered_cells <- WhichCells(scRNA4SeJeFBO, expression = Nucb2 > 0)
ra=length(filtered_cells)
filtered_cells <- WhichCells(scRNA4SeJeFBO, expression = Igf1r > 0)
rb=length(filtered_cells)
filtered_cells <- WhichCells(scRNA4SeJeFBO, expression = Igf1r > 0&Nucb2 > 0)
rc=length(filtered_cells)
filtered_cells <- WhichCells(scRNA4SeJeFBO, expression = Igf1r == 0&Nucb2 == 0)
rd=length(filtered_cells)
a=ra-rc 
b=rb-rc 
df=data.frame(name=sprintf(c('FBO','FBO','FBO','FBO'),gene,gene,gene,gene),Type=sprintf(c('%s+Igf1r-','%s-Igf1r+','%s+Igf1r+','%s-Igf1r-'),gene,gene,gene,gene),Num=c(a,b,rc,rd))
df$Type=factor(df$Type,levels=c(sprintf(c('%s-Igf1r-','%s-Igf1r+','%s+Igf1r-','%s+Igf1r+'),gene,gene,gene,gene)))
FBONIdata = ddply(df,'name',transform,percent_Num=Num/sum(Num)*100)
p <- ggplot(FBONIdata,aes(x=name,y=percent_Num,fill=Type))+geom_bar(stat = 'identity',width = 0.5,colour='black')+theme_classic()+labs(x='',y='Percentage')+theme(axis.title = element_text(size=20),axis.text = element_text(size=20))+scale_y_continuous(breaks=seq(0,100,25),labels=c('0','25%','50%','75%','100%'))+geom_text(aes(label=paste0(sprintf("%.1f", percent_Num), "%")),position = position_stack(vjust = 0.5), size = 8,color="black")+scale_fill_manual(values = c("#FB8072", "#1965B0", "#7BAFDE", "#882E72"))+theme( legend.text = element_text(size = 18))
ggsave(filename = sprintf("scRNA4SeJeFBO_%s_Igf1r-cor.ratio.pdf",gene), plot = p, height = 6, width = 6)
