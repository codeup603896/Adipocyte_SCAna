...
for (i in 1:length(searchs)){
    ...
    for (num in 1:length(groups)){
        ...
        compareInteractions(cellchat_m, show.legend = F, group = (1:4),size.text = 20)
        ...
        par(mfrow = c(1,1), xpd=TRUE)
        netVisual_diffInteraction(cellchat_m, weight.scale = T,comparison = group,sources.use='AP')
        netVisual_diffInteraction(cellchat_m, weight.scale = T, measure = "weight",comparison = group,sources.use='AP')
        netVisual_diffInteraction(cellchat_m, weight.scale = T,comparison = group,targets.use='AP')
        ...
        par(mfrow = c(1,1), xpd=TRUE)
        netVisual_bubble(cellchat_m, sources.use =c('AP','FBO','EC'),targets.use = c('AP','FBO','EC'),comparison = group, angle.x = 45, font.size = 16) +
        theme(legend.title = element_text(size = 16), legend.text = element_text(size = 16))
        print(netVisual_bubble(cellchat_m, sources.use =c('AP','FBO','EC'),targets.use = c('AP','FBO','EC'),comparison =  group, angle.x = 45, font.size = 16)) +
        theme(legend.title = element_text(size = 16), legend.text = element_text(size = 16))
        dev.off()
      }
}
